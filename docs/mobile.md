...
`run.focus` (loopback only), `run.answer` (`option` or `text` for the run's `dialog`), `team.tidy`. Always send a `requestId` you generated; resend the same one after a lost answer.
