...
export class WeawrClient {
  ...
  /** Answer a run's question dialog: `{ option }` presses that option, `{ text }` types into "Type something.". Refused (409) when the dialog changed. */
  runAnswer(team: string, run: string, answer: { option?: number; text?: string }, opts: { requestId?: string } = {}) { return this.command<{ outcome: string; label: string }>('run.answer', { team, run, ...answer, ...opts }); }
